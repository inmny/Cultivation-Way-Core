...
public class WindowItemLibrary : AutoLayoutWindow<WindowItemLibrary>
{
    private static WindowItemLibrary Instance;
    public List<ItemData> saved_item_datas = new();
    ...
    protected override void Init()
    {
        ...
        _item_stage_groups = new AutoGridLayoutGroup[Constants.Core.item_level_count / Constants.Core.item_level_per_stage];
        ...
        for (int i = 0; i < _item_stage_groups.Length; i++)
        {
            ...
            _item_pools[i] = new ObjectPoolGenericMono<SimpleEquipmentButton>(SimpleEquipmentButton.Prefab, _item_stage_groups[i].transform);
    ...
    {
        Instance.saved_item_datas.Add(pItemData);
    }
    public override void OnNormalEnable()
    {
        ...
        foreach (var item in Manager.items.list)
        {
            ...
            SimpleEquipmentButton button = _item_pools[item_data.Level / Constants.Core.item_level_per_stage].getNext(0);
            button.Setup(item_data, new Vector2(32, 36));
        }
        foreach (ItemData item in saved_item_datas)
        {
