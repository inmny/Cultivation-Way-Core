    ...
    internal class W_Content_Cultisys
    {
        public static void add_cultisys()
        {
            ...
            for (i = 0; i < Others.CW_Constants.max_cultisys_level; i++)
            {
                ...
                // 法抗
                immortal.bonus_stats[i].spell_armor = (int)(i * immortal.bonus_stats[i].base_stats.armor * (Mathf.Log(Utils.CW_Utils_Others.get_raw_wakan(immortal.bonus_stats[i].wakan, immortal.power_level[i]) / immortal.bonus_stats[i].wakan, Others.CW_Constants.wakan_level_co)+1))/2;
                immortal.bonus_stats[i].base_stats.armor *= (i+1)/2+1;
        ...
        private static bool immortal_level_judge(CW_Actor cw_actor, CW_Asset_CultiSys cultisys)
        {
            ...
            {
                cw_actor.cw_status.wakan = Utils.CW_Utils_Others.get_raw_wakan(cw_actor.cw_status.wakan, cw_actor.cw_status.wakan_level);

                cw_actor.cw_status.wakan_level = cultisys.power_level[cw_actor.cw_data.cultisys_level[cultisys.tag] + 1];

                cw_actor.cw_status.wakan = Utils.CW_Utils_Others.compress_raw_wakan(cw_actor.cw_status.wakan, cw_actor.cw_status.wakan_level);
            }
