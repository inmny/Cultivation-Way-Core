        ...
        public static bool actor_updateAge(Actor __instance)
        {
            ...
            */
            if (cw_actor.fast_data.health < cw_actor.cw_cur_stats.base_stats.health && cw_actor.has_cultisys(Others.CW_Constants.cultisys_bushido_tag))
            {
                cw_actor.get_fixed_base_stats().age_bonus -= cw_actor.cw_data.cultisys_level[1] + 1;
                // 允许血量超限
                cw_actor.fast_data.health += (int)Utils.CW_Utils_Others.transform_wakan((cw_actor.cw_data.cultisys_level[1] + 1) * (cw_actor.cw_data.cultisys_level[1] + 1) * cw_actor.cw_status.culti_velo,1,cw_actor.cw_status.health_level);
                cw_actor.check_level_up();
            }
